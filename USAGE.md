...
| 步骤 | 阶段 | 做什么 | 谁做 |
|------|------|--------|------|
| 1 | align | 需求对齐，产出对齐稿 | PM + AI（`/pm-align`） |
| 2 | design | 详细设计，产出设计基线 | AI（`/pm-design`） |
| 3 | design-review | 审查设计基线 | PM（`/pm-design-reviewer`） |
| 4 | prd | 生成 PRD 正文 | AI（`/pm-prd`） |
| 5 | prd-review | 审查 PRD | PM（`/pm-prd-reviewer`） |
| 6 | prototype | 生成 HTML 原型 | AI（`/pm-prototype`） |
| 7 | prototype-review | 审查原型 | PM（`/pm-prototype-reviewer`） |

每个 AI 生成步骤完成后，需运行 `stage-prep.py` 同步机读元数据。

## 步骤 1：需求对齐（align）

**PM 要做：** 使用 `/pm-align` 与 AI 多轮对话，确认需求理解、范围边界、角色场景。
...
**AI 要做：** 调用 `/pm-design`，基于 align 对齐稿生成设计基线（模块、页面、字段、状态、权限、规则）。
...
**PM 要做：** 调用 `/pm-design-reviewer`，AI 会自动：
...
**AI 要做：** 调用 `/pm-prd`，基于 design 基线生成 PRD 正文（详细需求说明、数据字典、权限汇总、状态机）。
...
**PM 要做：** 调用 `/pm-prd-reviewer`，AI 会自动执行预检查和正文质量审查（坏味道、三层覆盖、与 design 一致性）。
...
**AI 要做：** 调用 `/pm-prototype`，基于 design 基线生成 HTML 业务原型。
...
**PM 要做：** 调用 `/pm-prototype-reviewer`，AI 自动检查页面覆盖、状态表达、交互主路径、权限表现。
...
| `review-precheck.py --stage <阶段>` | reviewer 预检查（产物存在、章节完整、metadata 完整、ID 泄漏、lint） |
...
以 `stage-context.py` 输出为准（它实时计算）。如果仍不一致，检查 `status.json` 的 `current_stage` 是否正确。
