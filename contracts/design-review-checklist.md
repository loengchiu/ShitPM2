...
| X8. 推断值登记完整 | Design 正文出现默认值/排序/分页/提示文案/标签颜色等可推断值但 decision-notes 推断值清单无对应记录；同一失败/恢复话术跨场景机械重复 N 次；高影响项（权限/状态机/删除传播/外部系统/历史数据/数据层超时重试）被当推断值静默写入 | P1 | `content` / `consistency` |
