...
|---|---|---|---|---|---|
| 失败处理 | 无法判断归属层 | 用户指令模糊，可能是需求问题也可能是设计问题 | 命中该场景说明当前产物未满足对应要求 | 按最小判断清单 6 步逐项排查 | 停在澄清，输出当前判断和缺失信息，不直接改下游 |
| 失败处理 | 传播遗漏 | 改了 design 但忘了同步 PRD | 命中该场景说明当前产物未满足对应要求 | 传播方向表逐层检查 | 运行 prd-consistency-check.py 检查 PRD 与 design 一致性 |
| 失败处理 | 逆向定源 | PRD 直接改了 design 的语义 | 命中该场景说明当前产物未满足对应要求 | 识别到语义变更后，先停，回写 design | 若已误改 PRD，revert PRD 改动，先修 design |
| 失败处理 | fix 改了不该改的内容 | 表现层问题被误判为语义问题 | 命中该场景说明当前产物未满足对应要求 | 重新归类：表现问题只改 prototype，语义问题才回 design | 若已误改，按传播方向表反向恢复 |
| 失败处理 | Design 改了但旧确认未失效 | 修改 design.md 后忘记提示用户重新确认 | 命中该场景说明当前产物未满足对应要求 | 修改 design.md 后必须提示用户重新确认；下游 Skill 启动时重新计算哈希 | 运行 design-confirmation.py check 查看是否 hash_mismatch |
| 失败处理 | 上游缺失导致下游无法修复 | design 中缺少字段定义 | 命中该场景说明当前产物未满足对应要求 | 先在 design 中补字段定义 | 若字段来源不明，标记 [待确认] 回退 PM |
| 失败处理 | 多版本并存 | fix 覆盖后旧版本未清理 | 命中该场景说明当前产物未满足对应要求 | 确认每阶段只有一份当前稿 | 删除 output/<stage>/ 下的旧版本文件 |
| 失败处理 | 把下游内容提升为事实源 | PRD 或 Prototype 反向覆盖 Design | 命中该场景说明当前产物未满足对应要求 | 拒绝下游反向覆盖；以下游反馈为由先回写 Design | 若已误改，恢复 Design 原文，按传播方向重新同步 |
| 反模式 | **跳过影响分析直接改下游** | 出现该做法 | 可能遗漏受影响阶段，或改错归属层 | 必须按最小判断清单 6 步执行 | — |
