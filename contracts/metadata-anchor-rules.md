...
| 失败处理 | ID 泄漏到人读正文 | Design 或 prd.md 出现 FIELD-design-001 | 旧 metadata 机制未满足兼容约束 | 人工搜索正文中的稳定 ID，确认没有泄漏 | 手动搜索并删除正文中的 ID |
...
| 失败处理 | metadata 文件为空数组 | stage-prep.py 未提取到内容 | 旧 metadata 机制未满足兼容约束 | 检查 Design 对应文件章节是否有结构化表格 | 若表格格式被破坏，修复 Design 文件后重新生成 |
| 失败处理 | 多次 re-extract 后 ID 不稳定 | Design 表格顺序变化 | 旧 metadata 机制未满足兼容约束 | read_existing_entities() 按 title 匹配已有 ID | 若 title 也变了，视为新实体 |
| 失败处理 | page-fields 覆盖率不足 | 页面清单中有页面未出现在 page-fields.json | 旧 metadata 机制未满足兼容约束 | 检查 Design 的页面与字段落点章节是否遗漏 | 若页面无业务字段，声明 declared_empty |
...
| 失败处理 | metadata 与正文数量不一致 | 字段数/页面数/模块数对比偏差 | 旧 metadata 机制未满足兼容约束 | 运行 stage-prep.py --stage design 重新生成 | 若重新生成仍不一致，检查 Design 表格格式 |
...
| 反模式 | **把 metadata 当成产物质量证明** | 出现该做法 | ShitPM 旧 metadata 仅用于兼容诊断 | 以 Design 集合为唯一事实体系 | — |
...
| design-set.py | ShitPM：设计集校验、闭包、事务与下游依据 | 无 旧版兼容 对应 |

幻觉检测和语义一致性判断责任划分：
- **首次生成阶段**：PRD、Prototype 生成 Skill 必须在正式写入前完成与 Design 事实闭包的语义对照，覆盖核心对象、角色、状态、关键动作、流程、权限、模块和跨系统边界。这是生成责任，不是 Review 责任。
