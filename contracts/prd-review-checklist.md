...
| 18.1 页面展示行为完整（按需） | 无权限、加载中/骨架屏、空态、异常态、超长文本、默认值、标签颜色、分页/排序或筛选保留规则缺失 | P1 | `content` |
| 18.2 状态驱动展示完整（按需） | 状态页面缺可见/置灰/隐藏操作、字段显隐、点击跳转、标签颜色或刷新时机 | P1 | `content` / `consistency` |
...
| 25.1 自动动作失败闭环 | 自动生成/挂号/归档/迁移只有成功路径，缺失败状态、反馈、重试/补偿/人工处理或待确认 | P1 | `content` |
| 25.2 删除传播完整（按需） | 删除、软删除、停用、撤回、作废或归档未说明子对象、关联、审批、日志、附件、历史、引用和恢复/审计 | P1 | `content` |
| 25.3 枚举和独立上限有来源（按需） | 枚举只有“字典/枚举”占位，或分页、导出、批量、首页和文件限制没有分别表达 | P1 | `content` |
...
| 38. draw.io 交付完整 | 需要流程图但缺源文件、PNG，或图文名称/流程不一致 | P1 | `structure` / `content` |
