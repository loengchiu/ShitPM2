...
1. 是否为标签式正文（`**页面目标：**`、`**关键动作：**`、`**状态变化：**`、`**异常提示：**`、`**关联功能点：**`）
...
> PRD 生成阶段已用 `prd-consistency-check.py` 自检幻觉字段。以下检查项在 review 时抽查确认，重点关注脚本无法覆盖的语义层面一致性。
...
53. PRD 中出现但 design.md 中不存在的字段/页面为幻觉，必须修正（prd 生成阶段已用 prd-consistency-check.py 自检）
