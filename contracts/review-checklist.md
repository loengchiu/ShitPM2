...
7. 状态机闭环审查（按 `references/design-state-format.md` 闭环要求逐项查，违反即 P1）：
   ...
   - 操作人匹配业务角色：迁移的"操作人"与业务角色职责不一致即 P1
...
14. 页面内部是否按用户动作组织（动词短语），并用 `·` 并列项形成动作分块，非 UI 区域盘点
...
20. 列表、表单、详情中的字段描述是否只保留当前动作需要的关键字段、展示方式和规则，未把字段定义表完整字段清单原样搬进页面正文
...
40. 核心章节是否完整（名词说明、详细需求说明）
41. 字段定义表是否使用轻量格式（默认字段、类型、必填、说明；额外属性仅在确实影响实现时写入说明）
42. 字段定义表是否按实体分组，且额外属性没有无差别平铺为固定大宽表
43. 状态机是否按核心业务对象组织，包含状态集合、迁移、触发动作和限制条件
44. 验收标准汇总、风险与待确认是否仅在存在真实约束时出现；不存在时不保留空章节
45. 权限规则是否包含页面级、按钮级权限；如存在字段权限例外，是否已写入详细需求说明对应动作
...
47. 字段覆盖：design 字段定义中每个字段是否在字段定义表或页面章节中体现
48. 字段落地：字段定义表每个业务字段是否在详细需求说明、权限规则、状态流转、验收标准或风险待确认中有明确落点
49. 字段一致性：同一字段在不同页面的展示格式和校验规则是否一致
50. 权限一致性：权限规则与各页面按钮可见性/可操作性是否匹配
