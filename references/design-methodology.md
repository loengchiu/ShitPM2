...
字段较多是页面分区信号，不是删减字段的理由。不同来源、格式或校验的字段不得用斜杠、顿号等合并。展示、搜索、筛选、表单、统计、状态提示和操作依赖字段都属于产品事实，必须逐项定义；“基本字段”“相关信息”“按需展示”不是规格。

产品事实逐项完整，表达格式按复杂度适配：简单页面可使用紧凑清单、短表或自然语言；复杂页面和多角色流程使用详细字段表、操作表和状态机表。复杂规则按实际业务影响触发，普通单步查询和只读展示不被迫展开复杂模型。
