...
> **阅读须知（本库注入，非官方原文）**：本篇来自 Ant Design 官方设计团队规范，正文未改动（仅将上游跨仓库相对路径前缀去掉、改为同目录文件名引用，其余文字原样保留）。**颜色**请一律改用本库 Tabler 主题（官方 `#1677ff` / `#52c41a` / `#faad14` / `#ff4d4f` 等只作语义标签理解）；**`var(--xxx)`** 只取数值语义，不照抄变量名。完整转换规则与数值对照见同目录 `ADAPTATION.md`。
...
| 筛选项 **≥5 个**，或需要 **展开/收起**，或条件组合复杂、需分区展示 | **搜索卡承载 QueryFilter** | `components_Form.md` §5 筛选表单；代码模板 `scripts/form/05-QueryFilter.tsx` |

**选型优先级**（与 `components_Form.md` §5 不冲突时）：
