...
> **阅读须知（本库注入，非官方原文）**：本篇来自 Ant Design 官方设计团队规范，正文未改动。**颜色**请一律改用本库 Tabler 主题（官方 `#1677ff` / `#52c41a` / `#faad14` / `#ff4d4f` 等只作语义标签理解）；**`var(--xxx)`** 只取数值语义，不照抄变量名。完整转换规则与数值对照见同目录 `ADAPTATION.md`。
