...
> 本文件只在命中特定场景时读取。它负责组件选择、组合边界、稳定例外和跨层运行时契约；共享 UI 的名称、props、DOM、CSS 和默认实现以 templates/prototype-vite/src/shared/ui/ 源码为准，视觉值与 Token 以 prototype-visual-spec.md 为准，工程写法以 prototype-writing.md 为准，页面类型的标准结构以 prototype-page-types.md 为准，角色与权限表达以 prototype-role-permission.md 为准，通用完成门槛以 skills/spm-prototype/SKILL.md 为准。

## 目录

- 一、组件选择与组合判断
- 二、特定场景例外（表格与操作 / 表单与弹层 / Portal、响应式与回退）
- 三、跨层运行时契约
...
- 表格默认不固定列；只有宽度、阅读连续性或关键操作确有需要时才固定，并在真实浏览器中横向滚动复核固定列的遮挡、背景和层级。页面级的钉列/滚动判别按 prototype-page-types.md 总则的让路条款执行。
...
- 桌面端 Drawer 的操作区固定在 Header 右侧（确定/取消），长表单在内容区滚动：操作区不随内容滚出视口、不抖动、不改变高度；底部不再重复放一套提交按钮。
...
- Sider 只让菜单区滚动，遮罩、sticky ActionBar 与 Portal 使用统一层级；响应式按 <576、576–991、≥992、≥1200 检查，本轮不执行 390px 专项测试。
...
- JSX 使用的组件、图标和工具函数必须显式 import；图标一律从 shared/icons 取用，禁止直接 import 任何图标库。图标按钮必须有稳定 aria-label；可见操作必须有打开/关闭层、校验、状态/列表/路由变化或反馈。
