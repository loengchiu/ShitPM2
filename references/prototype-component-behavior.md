...
- 操作列最多直接展示 3 个文字操作；更多操作进入下拉。若 Design 要求不同的操作密度，说明业务原因并验证窄屏可操作性。
...
- DatePicker/RangePicker 的控件值使用 Dayjs；业务状态若保存字符串，必须同时完成回填转换和提交转换。
- Modal footer 使用 antd 内置 footer、调用方自定义 footer 或同一 flex 容器；不得用 float、零散块级布局或另一套按钮体系重造 footer。footer={null} 和自定义 footer 必须保持原语义。
