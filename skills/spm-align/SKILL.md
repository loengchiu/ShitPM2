---
name: spm-align
description: "可选需求整理模块——整理目标、范围、背景、已有材料和不确定点。ShitPM 中 Align 不是 Design 的硬前置；用户可跳过 Align 直接进入 Design。不承担最终业务流程、权限、状态和方案决策。"
---
...
ShitPM：Align 是**可选**的需求整理模块，不是 Design 的硬前置。
