...
材料准备由项目级运行时编排能力共享维护；Align 只消费编排器提供的事实资产，不拥有材料缓存、Design 阶段状态或下游动作规划。
...
宿主在 Align 开始前优先准备项目级材料资产。材料未变化时直接复用，不在 Align 中重复建立索引或全文读取；材料有变化时只摄取变化来源。
...
6. `.workflow/runtime/materials/manifest.json`、`source-index.json`（由 `$BUNDLE/scripts/python/source-index.py` 生成或复用）、`facts.json`（如存在）
7. 用户提供的背景材料、文件路径、链接和原话；仅在材料资产缺失或需要核验冲突时按索引行范围定点读取

## 执行流程

1. 优先从项目级材料事实资产中提取已确认事实，不重复读取同一原始材料；不要改写已确认范围。
