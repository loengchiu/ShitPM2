...
- 不修改 `output/design/design.md`、decision notes 或 metadata。
...
输出位于 `.workflow/runtime/design/review-precheck.json`。只有目标文件不存在、不可读或无法解析时停止；缺章节、内容不足、冲突、质量问题和 metadata 缺失必须继续审查并作为 finding。脚本误报 `can_start_review=false` 时人工核对文件可读性，可读则继续并记录 warning。
3. 运行 `python $BUNDLE/scripts/python/state-machine-check.py --project-root .`；无 legacy states 数据时按脚本降级到 `design.md`，解析失败时跳过结构层并保留业务层人审。
4. 读取 `$BUNDLE/contracts/review-checklist.md` 的 Design Review 部分，以及其中指定的 `design-state-format.md`、`design-writing.md` 等细则；按 checklist 审查字段密度、状态闭环、流程密度、权限、页面/字段落点、高影响缺口和旧 metadata（仅存在时）。
...
7. 输出 verdict 后停止，等待用户决定是否修复或确认。
...
- 共享 contract、schema 或必要脚本缺失：报告路径，不凭记忆补写完整清单。
