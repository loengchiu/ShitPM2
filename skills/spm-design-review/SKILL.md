...
**先读 design.md 全文**，后续通过 `--artifact-file` 让脚本直接读取产物文件。

### 第一段：预检查（结构完整性）

1. 运行 `python $BUNDLE/scripts/python/review-precheck.py --project-root . --stage design --artifact-file output/design/design.md` → `.workflow/runtime/design/review-precheck.json`
2.  脚本失败或 `can_start_review=false` → 停止输出阻塞项。假阳性（found_via_alias 且 blocking_issues 空）→ 列出 warnings 等用户确认后可继续
...
8. 不要求先通过其他 Review
9. 不承担计划内补全
10. 结论必须区分确定性问题、产品风险、待用户决策问题
11. 不运行 stage-prep.py 生成 metadata
12. 不自动调用 spm-fix 回写 Design（仅输出 needs_upstream_sync 建议）
