...
在进入页面和模块写作前，按实际适用性逐项判断以下事实属于已定义、局部定义、未定义或冲突，并把判断结果落实到对应 Design 文件、未决事项或上游冲突说明中：
...
按 `$BUNDLE/references/design-writing.md` 的“可推断值的写法”执行：低影响展示层细节直接写场景化取值和依据；权限、数据范围、角色、状态机、流程、生命周期传播、外部系统、历史数据、数据一致性、金额与统计口径、范围边界等高影响事项保持未决并单独询问。
