---
name: spm-fix
description: "同步修复：将变更影响沿事实链传播至唯一事实源并同步受影响下游。触发于用户要求同步修复、fix、回写上游或一致性修复；按传播契约局部修改，不整篇重写、不创建不存在的下游、无确认停顿。"
---
...
- 不能据此宣布的结论：Fix 完成仅代表变更已按传播契约同步，不能宣布整个系统所有模块均已通过 Review。
