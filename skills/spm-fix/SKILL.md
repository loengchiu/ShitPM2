---
name: spm-fix
description: "同步修复——ShitPM：把变更影响沿链路传播到当前事实源。用于用户说同步修复、fix、修复传播、回写上游、一致性修复，或 Review 建议回上游修复时。按传播 contract 局部修复，不整篇重写、不自动确认 Design、不自动生成所有下游。"
---
...
- 传播 contract 缺失：报告具体路径，不凭记忆扩展传播矩阵。
...
- 不自动确认 Design。
- 不自动生成所有下游。
- 不把 decision notes、metadata 或旧 Review 结果当作产品事实源。
- 不整篇重写与修改无关的产物。
