...
- 不修改 `output/prd/prd.md`、Design 或 decision notes，不自动调用 `spm-fix`。
...
预检查输出位于 `.workflow/runtime/prd/review-precheck.json`。只有目标文件不存在、不可读或无法解析时停止；缺章节、内容不足、冲突、写作质量问题和 metadata 缺失继续作为 finding。脚本误报 `can_start_review=false` 时人工核对文件可读性，可读则继续并记录 warning。
3. 运行：
...
直接引用 JSON 报告中的 `missing`、`hallucinated`、`attribute_mismatch`，再按 checklist 补充脚本覆盖不到的语义一致性、人读质量和 Design 未授权事实。
4. 读取 `$BUNDLE/contracts/review-checklist.md` 的 PRD Review 部分，以及 `$BUNDLE/references/prd-writing.md`、`prd-glossary-format.md`、`prd-versioning.md` 等被 contract 指定的细则。按 checklist 审查坏味道、三层覆盖、模块/页面/动作组织、字段/状态/权限落点、场景、幻觉和 Design 冲突。
...
6. 输出 verdict 后停止，不修改任何被审查产物。

## 判定规则

- 共享 contract 的统一门槛：零 P0/P1 为“通过”；零 P0 且 1 个 P1 为“有问题需修改”；存在 P0 或至少 2 个 P1 为“阻塞”。
- PRD 引入 Design 未授权字段、页面、状态、权限、流程、模块边界，或静默拍板 Design 的“待确认”事实时，必须作为高影响 finding，并建议回上游。
- 一致性校验是 Review finding 依据，不是阻止 Review 开始的门禁。
...
- 共享 contract、schema、references 或脚本缺失：报告路径，不凭记忆补写完整清单。
- 不运行 `stage-prep.py`，不生成 metadata，不修改 PRD、Design 或 decision notes。
