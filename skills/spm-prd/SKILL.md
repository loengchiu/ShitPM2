...
3. 大模块开头（模块职责段落之后、状态含义之前）写模块级权限规则：默认权限 + 例外字段权限。进模块先建立权限认知，不在每个动作重复默认可见角色
4. **小模块识别与分层**：
...
11. 列表、表单、详情中的字段描述只写当前动作真正需要读者理解的字段集合、展示方式和关键规则；不把数据字典完整字段、design 完整字段定义原样搬进页面正文，也不把表单字段原样平移成列表展示列
...
5. 表格只用于映射关系：字段定义、状态映射、权限规则、枚举定义用表格（归位到模块末尾的"模块数据"区）；区域说明、操作说明用自然语言或列表
...
10. **默认角色可见性不写**：按钮和动作的默认可见角色在模块末尾的"模块数据"权限规则区统一列示，详细需求正文不重复；只在有特殊限制时写明（如"仅编制人可见"、"仅项目主审可见"、"仅项目组长/主审可见"）
...
3. 字段定义、状态机、权限规则按模块归位到 §5 详细需求说明的"模块数据"区，是 design 事实源的交付镜像，不是新的事实源
