...
**表达规范**：具体数值写死；静态内容加引号，动态内容写来源；超长文本交代处理方式；标题下内容连续完整；不用伪排版；表格只用于映射关系和数据字典；UI 文案直接写在描述中。

**结构规范**：混用自然段（承接说明）、1. 2. 3.（顺序流程）、·（并列项）；动作下拆区块用 · 区块名：规则，子分组用 - ；业务流程只写业务流转，不写页面跳转。

**内容边界**：design 是字段/权限/状态的唯一事实源；PRD 不得独立新增或改写 design 已确认事实；数据字典是交付镜像区；权限汇总是集中展示区。

**禁止**：标签式正文（**页面目标：** 等）、动作流水账、纯表格式正文、模板腔、模糊表述。
