...
3. `$BUNDLE/references/prototype-writing.md`：通用后台基座、daisyUI 组件、Dashboard 布局、页面组织、shell 实现和视觉细则的唯一详细写法来源。
...
4. 读取并执行 `$BUNDLE/references/prototype-writing.md` 的通用后台基座、daisyUI、Dashboard、页面组织和 shell 规则；以 `$BUNDLE/templates/prototype.html` 为基础生成或局部修改。
...
读取反馈后必须先按 `$BUNDLE/templates/prototype-feedback-classification.md` 输出归类，再开始修改；表现问题与语义问题不能混改。
