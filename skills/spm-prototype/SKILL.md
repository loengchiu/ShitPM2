---
name: spm-prototype
description: "Prototype 阶段——根据已确认的 Design 生成可运行、可讨论的 HTML 原型。用于用户要求生成、修改或查看原型时；直接读取 Design，不依赖 PRD，必须保持 HTML + React 18 + Ant Design 6 + 本地 lib 架构，不自行引入 Design 未授权的高影响行为。"
---
...
- HTML + React 18 + Ant Design 6 + 本地 `lib/` 架构固定，不引入新的构建链、外部 CDN、Vue/daisyUI/`el-` 组件或 React 19。
...
4. `$BUNDLE/lib/`：本地 React 18 和 Ant Design 6 运行资源。
...
- 用户反馈无法归类：停止澄清，不直接修改。
- 不依赖 PRD 存在，不依赖 metadata，不使用外部 CDN、Vue/daisyUI/`el-` 组件或 Unix 专属命令描述来替代本地资源方案。
