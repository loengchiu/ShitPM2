...
10. 生成后依次检查：
   ...
   - **多角色页面必须有角色视角**：Design 页面清单"适用角色"多于一个角色的页面，必须提供角色切换控件（Select）或"无权限置灰"，使角色权限差异可观察；不得静默只按单一角色渲染；
   - **配置管理页不得用占位**：新增/编辑/停用/启用等操作必须用真实 `Modal` + `Form`（字段按 Design 对应页面章节），二次确认用 `Modal.confirm`，禁止 `message.info('…（示意）')` 占位；
   - **状态驱动操作必须可见禁用**：Design 状态机要求的操作可用条件必须用 antd `disabled` 属性（或等价置灰）表达，不得只写文字说明或全部可点；关键权限/状态/限制/主路径/异常反馈必须有表达。
