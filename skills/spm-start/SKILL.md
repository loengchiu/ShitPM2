---
name: spm-start
description: "项目启动与导航：只读扫描当前产物、Design 修改状态、下游受影响状态与可用动作。触发于查看项目状态、下一步指引或阶段导航；不修改任何文件，不给唯一下一步。"
---
...
6. 读取 $BUNDLE/contracts/start-action-matrix.md 匹配可用动作矩阵与依赖条件。
7. 按 $BUNDLE/templates/start-report.md 输出项目状态、产物清单、最近 Review、Design 修改状态、下游受影响模块、可用动作和建议。
8. 输出后停止。

完成判据：当前产物、Design 修改状态、下游受影响模块、可用动作已扫描并输出；未修改任何文件；未给唯一下一步。不能据此宣布任何产物已通过验收。
