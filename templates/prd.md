...
<!-- 核心：按模块 → 页面 → 动作组织 -->
<!-- 模块级规则：仅在存在真实跨页面流转、多角色协同或状态变更时，才在模块开头补充“详细流程图”和“状态变化图”；纯展示、纯配置管理或无业务状态变更的模块不补图，改为 1-2 句自然语言说明，细的流转与状态说明收敛在详细需求说明内 -->
<!-- 推荐骨架：
### （一）模块名
模块职责与涉及页面，1-2 句
...
<!-- 核心：按核心业务对象组织 -->
<!-- 每个实体必须包含：状态流转速览（一行路径）+ 状态明细表（状态值/名称/进入条件/退出条件）+ 逐条迁移规则（触发角色、前置条件、状态变化、是否可逆、关键副作用） -->
