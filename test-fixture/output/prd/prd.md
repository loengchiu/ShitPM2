...
进入"进行中"后，准备工作与实施工作并行推进。准备工作包括实施方案、审计通知书、进点会记录、承诺书、资料清单 5 项材料，每项均支持多次修改重提。实施工作从底稿录入开始：组员录入底稿→项目组长/主审初审→被审单位反馈意见→组内复核→复核通过后自动生成审计问题；问题设定上报告标记后，可在上报告问题中被引用归纳，提交组内审核。
...
签署情况列表展示被审单位用户对接人、签署时间、上传时间、签署文件。发出承诺书后生成签署记录，签署方式根据对接人是否有系统账号自动判定（有账号→线上签署，无账号→附件签署）；附件签署时可上传签字盖章版文件。
...
登录表单提供用户账号、密码输入框（必填）和"登录"按钮。验证通过后根据用户角色跳转对应首页：审计人员→审计单位首页；被审单位人员→被审单位首页；管理员→系统管理首页。
...
树形列表展示机构名称、机构编码、上级机构、排序号、状态、是否审计部门。顶层为集团，下辖子公司和集团本部部门。支持拖拽排序。组织结构层级：集团 → 子公司/集团本部部门 → 部门/小组。
